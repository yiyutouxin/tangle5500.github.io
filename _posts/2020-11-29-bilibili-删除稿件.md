...
url = "https://member.bilibili.com/platform/upload-manager/article?order=click&page=5500" # 全部稿件
# url = "https://member.bilibili.com/platform/upload-manager/article?order=click&page=1&group=not_pubed" # 未通过
driver.get(url)
print(driver.title)
...
def checkVeriImage(driver):
    ...
    len_im_bytes = len(im_bytes)
    print(len_im_bytes)
    if len_im_bytes <= 84804:                             # 小电视
        driver.find_element_by_css_selector(".geetest_refresh_1").click()
        checkVeriImage(driver)
    if len_im_bytes <= 110373 and len_im_bytes >= 109907: # 火锅
        driver.find_element_by_css_selector(".geetest_refresh_1").click()
        checkVeriImage(driver)
    if len_im_bytes <= 106873 and len_im_bytes >= 105698: # 兔子
        driver.find_element_by_css_selector(".geetest_refresh_1").click()
        checkVeriImage(driver)
...
def task():
    for i in range (10000):
        try:
            ...
            if url[-4:] == "5500":
                if wait_if(".more-btn"):
                    ele = driver.find_element_by_class_name('more-btn')
                    ActionChains(driver).move_to_element(ele).perform()        # 鼠标移到悬停元素上
                if wait_if(".option-text"):
                    driver.find_element_by_class_name('option-text').click()   # 删除稿件（全部稿件）
            else:
                if wait_if(".bili-btn"):
                    driver.find_elements_by_class_name('bili-btn')[1].click()  # 删除稿件（未通过）
            if wait_if(".geetest_radar_tip"):
