...
| 结构化     | 元组   | 共有 9个元素组                                              |
| 格式化时间 | 字符串 | 已格式化的结构使时间更具可读性包括自定义格式和固定格式   |
| 延时       | 无     | 延时                                                       |
...
tm_hour  # 时（0-23）
tm_min   # 分（0-59）
tm_sec   # 秒（0-61）60 or 61 为润秒
tm_wday  # 天（0-6）星期一为星期的开始
tm_yday  # 天（1-366）
tm_isdst # 夏令时（0-1）
```
...
time.ctime(秒)            # 本地固定格式时间，添加参数从 1970年计时
...
| %M   | 59                       | 分（00-59）                     |
| %S   | 59                       | 秒（00-59）                     |
| %c   | Thu Feb 11 18:10:02 2021 | 格式化时间                      |
| %x   | 02/11/21                 | 日期                            |
| %X   | 18:11:37                 | 时间                            |
| %B   | February                 | 月                              |
| %b   | Feb                      | 月                              |
...
| %j   | 042                      | 天（001-366）                   |
| %p   | PM                       | AM or PM                        |
| %U   | 06                       | 星期（00-53）星期天为星期的开始 |
| %w   | 4                        | 星期（0-6）星期天为星期的开始   |
| %W   | 06                       | 星期（00-53）星期一为星期的开始 |
| %Z   | PST                      | 当前时区的名称                  |
| %%   | %                        | %号本身                         |
| %y   | 21                       | 年份（00-99）                   |
| %I   | 06                       | 时（01-12）                     |
...
```
time.strptime('2018-12-17','%Y-%m-%d') # 格式化 — 结构化
time.strftime('%Y-%m-%d',结构化)       # 结构化 — 格式化
time.localtime(时间戳)                 # 时间戳 — 结构化
time.ctime(时间戳)                     # 时间戳 — 格式化
time.asctime(结构化)                   # 结构化 — 格式化
time.mktime(格式化)                    # 结构化 — 时间戳
```
...
datetime.now()                               # 2018-12-16 16:45:20.194687
datetime.now().strftime("%Y-%m-%d %H:%M:%S") # 2018-12-16 16:45:20
```
...
```
sec = 1613037934
test = datetime.fromtimestamp(sec)
var = test.strftime("%Y-%m-%d %H:%M:%S")
print(var) # 2021-02-11 18:05:34
```

### 字符串转格式化时间

```
string = '2021-02-11 17:23:42'
test = datetime.strptime(string, '%Y-%m-%d %H:%M:%S') # 2021-02-11 17:23:42
print(test) # 2021-02-11 18:05:34
```
...
string = '2000-10-10 10:10:10'
date = datetime.strptime(string, '%Y-%m-%d %H:%M:%S') # 2000-10-10 10:10:10
date += timedelta(days=10)                            # 2000-10-20 10:10:10
```
...
| 语法                                    | 返回值 | 作用                               |
| --------------------------------------- | ------ | ---------------------------------- |
| `os.path.split(r'F:\Tangle\1.py')`    | 元组   | 将路径分解为目录和文件名         |
| `os.path.join(r'F:\Tangle','666')`     |        | 拼接路径                           |
| `os.path.dirname(r'F:\Tangle\1.py')`  |        | 返回路径中的目录部分结尾不包括 `\`  |
| `os.path.basename(r'F:\Tangle\1.py')` |        | 返回路径中的文件名                 |

## 查看文件时间

| 语法                                    | 返回值 | 作用                       |
| --------------------------------------- | ------ | -------------------------- |
| `os.path.getmtime(r'F:\Tangle\1.py')` | 时间戳 | 文件或文件夹的最后修改时间 |
| `os.path.getatime(r'F:\Tangle\1.py')` | 时间戳 | 文件或文件夹的最后访问时间 |
| `os.path.getctime(r'F:\Tangle\1.py')` | 时间戳 | 文件或文件夹的创建时间     |

## 查看文件大小

| 语法                                   | 作用                                            |
| -------------------------------------- | ----------------------------------------------- |
| `os.path.getsize(r'F:\Tangle\1.py')` | 文件或文件夹的大小，若是文件夹返回 0，字节类型   |

## 查看文件是否存在

| 语法                                  | 返回值        | 作用             |
| ------------------------------------- | ------------- | ---------------- |
| `os.path.exists(r'F:\Tangle\1.py')` | `True | False` | 查看文件是否存在 |
...
返回模块的搜索路径，初始化使用 Python 环境变量的值

## sys.modules

是一个全局字典，该字典是 python 启动后就加载在内存中，每当程序员导入新的模块，sys.modules 将自动记录该模块，当第二次再导入该模块时，python 会直接到字典中查找，从而加快了程序运行的速度，它拥有字典所拥有的一切方法
...
只在字符串开始匹配,返回一个对象，对象可以调用 group，返回结果
...
- 安装 Microsoft Visual C++ 14.0 选择 Windows 8.1 SDK 功能

- 复制 C:\Program Files (x86)\Microsoft Visual Studio 14.0\VC\include\stdint.h

- 粘贴 C:\Program Files (x86)\Windows Kits\10\Include\10.0.10240.0\ucrt

- 修改 C:\Program Files (x86)\Windows Kits\10\Include\10.0.10240.0\ucrt\inttypes.h
...
- 控件和放置最好两行代码，不然有可能返回 None

- 控件事件函数需要两个参数

- 导入模块顺序 tkinter 要在 PIL 前
...
DSN = 'PROVIDER = Microsoft.Jet.OLEDB.4.0;DATA SOURCE = C:\\Users\\Tangle\\Desktop\\test.mdb'
conn.Open(DSN)
...
- 教程 <https://docs.python.org/zh-cn/3.7/howto/argparse.html>
...
def main():
    ...
    parser.add_argument('-a','--age', default='20', type=int, help='text', nargs='?')
    parser.add_argument('-d','--debug', action='store_true')
    args = parser.parse_args()
    print(args)                    # Namespace(age=20, debug=False, name='Tangle')
    name = args.name
    age = args.age
    debug = args.debug
    print(f'{name} {age} {debug}') # Tangle 20 False
...
async def main(i):
    await asyncio.gather(hello(i), hello(i+0.1))
    await hello(i+0.2)

loop = asyncio.get_event_loop() # 获取事件循环
arg_list = [1, 2]               # 参数列表
task = [asyncio.ensure_future(main(i)) for i in arg_list] # 任务列表
loop.run_until_complete(asyncio.wait(task))               # 任务列表同时运行直到完成
loop.close()                    # 关闭事件循环
```
