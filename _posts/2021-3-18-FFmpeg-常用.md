...
```
ffmpeg -y -i input.mp4 -i watermark.png -filter_complex [1:v]colorchannelmixer=aa=0.3[valpha];[0:v][valpha]"overlay='if(ld(0), if(lte(mod(t/5,1),0.05),st(0,0);NAN,ld(1)), st(0,1);ld(1);st(1,random(time(0))*(W-w));NAN)':'if(ld(0), if(lte(mod(t/5,1),0.05),st(0,0);NAN,ld(1)), st(0,1);ld(1);st(1,random(time(0))*(H-h));NAN)'" -c:v h264_nvenc -s 1920x1080 -r 60 -b:v 5.9M -bufsize 2.95M -minrate 5.9M -maxrate 5.9M temp.mp4
```
