...
```
# 全屏随机
ffmpeg -y -i qq.mp4 -i qq.png -filter_complex "overlay='if(ld(0), if(lte(mod(t/10,1),0.05),st(0,0);NAN,ld(1)), st(0,1);st(1,random(t)*(W-w));NAN)':'if(ld(0), if(lte(mod(t/10,1),0.05),st(0,0);NAN,ld(1)), st(0,1);if(lte(random(t),0.3),st(1,0),st(1,random(t)*(H-h)));NAN)'" -c:v h264_nvenc output.mp4
```
