...
```
ffmpeg -i input.avi -c:v libx264 -s 1920x1080 -r 60 -b 5.9M output.mp4
ffmpeg -i input.avi -c:v libx264 -preset medium -crf 23 -pix_fmt yuv420p -s 1920x1080 -r 60 -b 5.9M output.mp4
```

## 格式转换

```
for /r %a in ("*.mkv") do ffmpeg -hwaccel cuvid -i "%a" -c:v h264_nvenc "%~pna.mp4"
```
...
```
ffmpeg -y -t 30 -i 1080p+.mp4 -i 水印.png -filter_complex [1:v]colorchannelmixer=aa=0.3[valpha];[0:v][valpha]overlay=lt(mod(t\,20)):x='(W-w)/2':y='mod(t,20)*60-h' -c:v h264_nvenc -s 1920x1080 -r 60 -b 5.9M move.mp4
```
