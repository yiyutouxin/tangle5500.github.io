...
```
self.start_date = datetime.now().strftime('%Y-%m-%d')                         # 修改前
 
self.start_date = datetime.now().strftime('%Y-%m-%d')                         # 修改后
self.start_date = time.strftime("%Y-%m-%d",time.localtime(time.time()-86400)) # 修改后
```

```
if since_date <= today:      # 修改前

if str(since_date) <= today: # 修改后
```
